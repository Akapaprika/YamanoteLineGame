...
logger = get_logger('answer_list')


@dataclass
class AnswerList:
    """Manages answer items with matching and usage tracking."""
    ...
    # Cache for remaining items to optimize lookups
    _remaining_cache: Optional[List[str]] = field(default=None, init=False)
    ...
    def load_from_csv(cls, path: str, encoding: str = "utf-8") -> "AnswerList":
        ...
        try:
            ...
            logger.error(f"IO error reading CSV: {path}", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Error reading CSV: {path}", exc_info=True)
            raise
        ...
        if separator_idx is not None:
            for row in rows[separator_idx + 1:]:
                ...
                try:
                    ...
                    logger.warning(f"Error processing answered row {row}: {e}")
                    continue
    ...
    def find_match(self, text: str, previous_class: str = None) -> Optional[str]:
        ...
        2. Short-form match (element_map): matches only the city reading when
           previous_class matches the class_map of the candidate
        """
    ...
    def mark_used(self, text: str, previous_class: str = None) -> Optional[str]:
        ...
        if matched:
            ...
            self._remaining_cache = None  # Invalidate cache
            return matched
        return None

    def remaining_count(self) -> int:
        """Get count of remaining (unanswered) items."""
    ...
    def save_to_csv(self, path: str, encoding: str = "utf-8") -> None:
        ...
        try:
                ...
                for norm_disp in self.items:
                    if norm_disp not in self.used:
                        if norm_disp in self._display_map:
                            ...
                            # Fallback if _display_map missing
            ...
            logger.error(f"Save directory not found: {path}", exc_info=True)
            raise
        except IOError as e:
            logger.error(f"IO error saving CSV: {path}", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Error saving CSV: {path}", exc_info=True)
            raise
